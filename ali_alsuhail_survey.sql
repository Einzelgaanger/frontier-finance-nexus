...
-- Delete any existing 2021 survey response for Ali Al Suhail
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'ali.alsuhail@kapita.iq');

-- Insert complete 2021 survey response for Ali Al Suhail
...
SELECT
  ...
  ARRAY['Fund partners', 'Angel network', 'Development Finance Institutions (DFIs)', 'Donors/philanthropy', 'Corporates'],
  '',
  'Targeted',
  'Financial return first',
  ARRAY['Youth', 'Job creation'],
  false,
  NULL,
  NULL,
  NULL,
  ...
  ARRAY['No'],
  '',
  '2021-02-01 10:47:53'::timestamp,
  now(),
  now()
FROM auth.users u
WHERE u.email = 'ali.alsuhail@kapita.iq';
