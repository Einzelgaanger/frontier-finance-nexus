...
-- Delete any existing 2021 survey response for Owen E Harmon
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'oeharmon@gemicap.com');

-- Insert complete 2021 survey response for Owen E Harmon
...
SELECT
  ...
  'Target',
  '8+ years',
  ARRAY['Niche Ventures (innovative products/services targeting niche markets with growth ambitions)', 'High-Growth Ventures (disruptive business models targeting large markets with high growth potential)'],
  ARRAY['Start-up / pre-seed (pre-revenue, concept and business plan development)', 'Early stage / seed (early revenue, product/service development, funds needed to expand business model)', 'Growth (established business in need of funds for expansion, assets, working capital etc)'],
  ...
  ARRAY['Fund partners', 'Development Finance Institutions (DFIs)'],
  ...
  ARRAY['Common equity', 'Preferred equity (e.g. certain rights above those available to common equity holders)', 'Convertible notes', 'Shared revenue/earnings instruments', 'a mix of equity & convertible debt to also be considered'],
  ...
  ARRAY['No'],
  ARRAY['Stabilize operations of existing portfolio companies', 'Growth capital for existing portfolio to increase market share', 'Growth capital for existing portfolio to enter new markets/expand business line(s)', 'New pipeline investments through existing vehicle', 'Technical assistance to support portfolio enterprises'],
  ARRAY['Seek increased access to new LP funds internationally', 'Increase use of technology in order to lower fund operational costs', 'Increase use of data and technology to facilitate investment decisions', 'Build new partnerships for joint co-investment opportunities, expand pipeline opportunities'],
  '2',
  jsonb_build_object(
    'Fund Economics', 3,
    'LP Profiles', 2,
  ...
  'Not sure',
  ARRAY['Yes, open ended vehicles', 'Yes, early stage debt vehicles', 'Yes, early stage equity', 'Yes, gender lens investing'],
  '',
  '2021-02-01 13:37:18'::timestamp,
  now(),
  now()
FROM auth.users u
WHERE u.email = 'oeharmon@gemicap.com';
