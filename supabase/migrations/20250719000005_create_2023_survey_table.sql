-- Create 2023 MSME Financing Survey responses table
...
CREATE INDEX IF NOT EXISTS idx_survey_responses_2023_user_id ON survey_responses_2023(user_id);
CREATE INDEX IF NOT EXISTS idx_survey_responses_2023_organisation_name ON survey_responses_2023(organisation_name);
CREATE INDEX IF NOT EXISTS idx_survey_responses_2023_created_at ON survey_responses_2023(created_at);
CREATE INDEX IF NOT EXISTS idx_survey_responses_2023_completed_at ON survey_responses_2023(completed_at);

-- Enable Row Level Security
ALTER TABLE public.survey_responses_2023 ENABLE ROW LEVEL SECURITY;

-- Create RLS policies
CREATE POLICY "Users can view their own 2023 survey responses" ON public.survey_responses_2023
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own 2023 survey responses" ON public.survey_responses_2023
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own 2023 survey responses" ON public.survey_responses_2023
  FOR UPDATE USING (auth.uid() = user_id);

-- Admin policy for viewing all responses
CREATE POLICY "Admins can view all 2023 survey responses" ON public.survey_responses_2023
  FOR SELECT USING (
    EXISTS (
...
CREATE TRIGGER update_survey_responses_2023_updated_at 
