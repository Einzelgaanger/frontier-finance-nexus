...
-- Delete any existing 2021 survey response for Victor Ndiege
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'victor.ndiege@kcv.co.ke');

-- Insert complete 2021 survey response for Victor Ndiege
...
SELECT
  u.id,                                                          -- user_id
  'Kenya Climate Ventures',                                      -- firm_name
  'Victor Ndiege',                                               -- participant_name
  'CEO',                                                         -- role_title
  ARRAY['Africa - East Africa'],                                 -- team_based
  ARRAY['Africa - East Africa', 'Africa - Central Africa'],      -- geographic_focus
  ...
  'Second',                                                     -- top_sdg_2
  'Others',                                                     -- top_sdg_3
  ARRAY['Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Requirement', 'Investment Requirement', 'Investment Requirement', 'Investment Consideration', 'Investment Consideration']::text[], -- gender_considerations_investment
  ARRAY['Require specific reporting on gender related indicators by your investees/borrowers']::text[], -- gender_considerations_requirement
  ...
  ARRAY['Convertible notes', 'Senior debt', 'Mezzanine debt'],  -- investment_forms
  ARRAY['Agriculture / Food supply chain', 'Energy / Renewables / Green Mobility', 'Financial Inclusion / Insurance / Fintech', 'Water and Sanitation'], -- target_sectors
  ...
  ARRAY['Management buyout', 'Financial investor take-out'],    -- investment_monetization
  '1 exit negotiated for April 2021.',                          -- exits_achieved
  ...
  ARRAY['Seek increased access to new LP funds internationally', 'Increase application of alternative debt instruments (e.g. mezzanine debt, convertible debt, or shared revenue instruments)', 'Increase use of technology in order to lower fund operational costs', 'Build new partnerships for joint co-investment opportunities, expand pipeline opportunities'], -- fund_vehicle_considerations
  ...
  'Approaches to deployment of TA& BDS to portfolio companies.', -- new_working_group_suggestions
  ...
  'Approaches for deployment of TA& BDS, considering fund economics perspective.', -- new_webinar_suggestions
  ...
  '',                                                           -- additional_comments
  '2021-02-11 08:35:13'::timestamp,                            -- completed_at
  now(),                                                        -- created_at
  now()                                                         -- updated_at
FROM auth.users u
WHERE u.email = 'victor.ndiege@kcv.co.ke';
