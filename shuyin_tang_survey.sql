...
-- Delete any existing 2021 survey response for Shuyin Tang
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'shuyin@beaconfund.asia');

-- Insert complete 2021 survey response for Shuyin Tang
...
SELECT
  u.id,                                                          -- user_id
  'Beacon Fund',                                                 -- firm_name
  'Shuyin Tang',                                                 -- participant_name
  'CEO',                                                         -- role_title
  ...
  'First deal approved by our IC in February 2021',             -- optional_supplement
  ...
  ARRAY['Senior debt'],                                         -- investment_forms
  ...
  ARRAY['Increase application of alternative debt instruments (e.g. mezzanine debt, convertible debt, or shared revenue instruments)', 'Increase use of technology in order to lower fund operational costs', 'Increase use of data and technology to facilitate investment decisions'], -- fund_vehicle_considerations
  ...
  'Fund structuring - particularly for open-ended vehicles',    -- new_working_group_suggestions
  ...
  '',                                                           -- additional_comments
  '2021-02-10 04:37:50'::timestamp,                            -- completed_at
  now(),                                                        -- created_at
  now()                                                         -- updated_at
FROM auth.users u
WHERE u.email = 'shuyin@beaconfund.asia';
