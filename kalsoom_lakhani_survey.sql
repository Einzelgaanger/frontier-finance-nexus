...
-- Delete any existing 2021 survey response for Kalsoom Lakhani
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'klakhani@Invest2innovate.com');

-- Insert complete 2021 survey response for Kalsoom Lakhani
...
SELECT
  u.id,                                                          -- user_id
  'i2i Ventures',                                                -- firm_name
  'Kalsoom Lakhani',                                             -- participant_name
  'Co-Founder/GP',                                              -- role_title
  ARRAY['Asia - South Asia'],                                    -- team_based
  ARRAY['Asia - South Asia'],                                    -- geographic_focus
  ...
  ARRAY['Venture launch – invest in initial staff, product/services development and market acceptance', 'Enterprise growth capital (e.g. intangible investments such as staff build-out, expanded sales & marketing capabilities, new markets, operational and support systems, etc.)'], -- financing_needs
  ARRAY['Fund partners', 'High Net Worth Individuals (HNWIs)', 'Angel network', 'Development Finance Institutions (DFIs)', 'Impact investing family offices', 'Donors/philanthropy'], -- target_capital_sources
  'Targeted',                                                   -- target_irr_achieved
  'Responsible/ESG investing (negative screening), Impact investing (positive screening)', -- target_irr_targeted
  'Responsible/ESG investing (negative screening), Impact investing (positive screening)', -- impact_vs_financial_orientation
  ARRAY['Gender', 'Job creation'],                              -- explicit_lens_focus
  false,                                                        -- report_sustainable_development_goals
  NULL,                                                         -- top_sdg_1
  NULL,                                                         -- top_sdg_2
  NULL,                                                         -- top_sdg_3
  ARRAY['Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Requirement', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration']::text[], -- gender_considerations_investment
  ...
  ARRAY['Common equity', 'Convertible notes', 'SAFEs'],         -- investment_forms
  ARRAY['Sector agnostic', 'Technology / ICT / Telecommunications'], -- target_sectors
  ...
  ARRAY['Other types of self-liquidating repayment structures', 'Strategic sale/merger of company', 'Management buyout', 'Financial investor take-out'], -- investment_monetization
  '0',                                                          -- exits_achieved
  ...
  ARRAY['New pipeline investments through existing vehicle', 'New pipeline investments through new vehicle'], -- raising_capital_2021
  ARRAY['Seek increased access to new LP funds internationally', 'Increase use of data and technology to facilitate investment decisions', 'Build new partnerships for joint co-investment opportunities, expand pipeline opportunities'], -- fund_vehicle_considerations
  ...
  'no',                                                         -- new_working_group_suggestions
  ...
  'Yes, as a mentor',                                           -- participate_mentoring_program
  ARRAY['No']::text[],                                          -- present_demystifying_session
  '',                                                           -- additional_comments
  '2021-03-15 02:36:55'::timestamp,                            -- completed_at
  now(),                                                        -- created_at
  now()                                                         -- updated_at
FROM auth.users u
WHERE u.email = 'klakhani@Invest2innovate.com';
