...
-- Delete any existing 2021 survey response for Karthik Chandrasekar
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'karthik@sangam.vc');

-- Insert complete 2021 survey response for Karthik Chandrasekar
...
SELECT
  u.id,                                                          -- user_id
  'Sangam Ventures',                                             -- firm_name
  'Karthik Chandrasekar',                                        -- participant_name
  'CEO',                                                        -- role_title
  ...
  ARRAY['Venture launch – invest in initial staff, product/services development and market acceptance', 'Major capital investments (facilities, production equipment, fleet/logistics, etc.)', 'Inventory and working capital requirements'], -- financing_needs
  ARRAY['Development Finance Institutions (DFIs)', 'Bilateral agencies', 'Impact investing family offices', 'Donors/philanthropy', 'Corporates'], -- target_capital_sources
  'Targeted',                                                   -- target_irr_achieved
  'Achieved in most recent reporting period',                   -- target_irr_targeted
  'Climate change focus with financial return and scale targets', -- impact_vs_financial_orientation
  ARRAY['Climate / green ventures'],                            -- explicit_lens_focus
  true,                                                         -- report_sustainable_development_goals
  'Third',                                                      -- top_sdg_1
  'Third',                                                      -- top_sdg_2
  'Third',                                                      -- top_sdg_3
  ARRAY['Investment Consideration', 'Investment Requirement', 'Investment Consideration', 'Investment Requirement']::text[], -- gender_considerations_investment
  ...
  ARRAY['Agriculture / Food supply chain', 'Energy / Renewables / Green Mobility', 'Manufacturing', 'Technology / ICT / Telecommunications', 'Circular economy and resource productivity'], -- target_sectors
  ...
  jsonb_build_object(                                            -- covid_impact_portfolio
    ...
    'Need to pivot business model', 'Anticipate future impact'
  ),
  ARRAY['No'],                                                  -- covid_government_support
  ARRAY['Stabilize operations of existing portfolio companies', 'Growth capital for existing portfolio to increase market share', 'New pipeline investments through existing vehicle'], -- raising_capital_2021
  ...
  'Fund structuring and PPM design',                            -- new_working_group_suggestions
  jsonb_build_object(                                            -- webinar_content_ranking
    ...
    'Human Capital (peer discussion)', 4,
    'Co-investing workshop with ADAP (peer discussion)', 5,
  ...
  'Not for now',                                                -- new_webinar_suggestions
  ...
  'Yes, as a mentor, Yes, as a mentee',                         -- participate_mentoring_program
  ARRAY['Too late']::text[],                                    -- present_demystifying_session
  '',                                                           -- additional_comments
  '2021-03-11 05:44:12'::timestamp,                            -- completed_at
  now(),                                                        -- created_at
  now()                                                         -- updated_at
FROM auth.users u
WHERE u.email = 'karthik@sangam.vc';
