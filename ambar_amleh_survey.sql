...
-- Delete any existing 2021 survey response for Ambar Amleh
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'ambar@ibtikarfund.com');

-- Insert complete 2021 survey response for Ambar Amleh
...
SELECT
  ...
  ARRAY['Venture launch – invest in initial staff, product/services development and market acceptance', 'Enterprise growth capital (e.g. intangible investments such as staff build-out, expanded sales & marketing capabilities, new markets, operational and support systems, etc.)'],
  ARRAY['High Net Worth Individuals (HNWIs)', 'Development Finance Institutions (DFIs)', 'Donors/philanthropy', 'Corporates'],
  ...
  'First',
  'Second',
  ARRAY['Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration']::text[],
  ARRAY['Female staffing is ≥ 50%', 'Provide specific reporting on gender related indicators for your investors/funders', 'Require specific reporting on gender related indicators by your investees/borrowers']::text[],
  ARRAY[]::text[],
  ...
  ARRAY['Yes, grant funding (financial)'],
  ARRAY['New pipeline investments through new vehicle', 'hopefully closing fund 2'],
  ...
  'Unfortunately, I was not able to join the working groups, hopefully I can soon.',
  jsonb_build_object(
  ...
  '',
  '2021-02-03 09:42:12'::timestamp,
  now(),
  now()
FROM auth.users u
WHERE u.email = 'ambar@ibtikarfund.com';
