...
-- Delete any existing 2021 survey response for Ebenezer Arthur
DELETE FROM public.survey_responses_2021 
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'e.arthur@wangaracapital.com');

-- Insert complete 2021 survey response for Ebenezer Arthur
...
SELECT
  u.id,                                                          -- user_id
  'Wangara Green Ventures',                                      -- firm_name
  'Ebenezer Arthur',                                             -- participant_name
  'FOunder & CEO',                                              -- role_title
  ...
  ARRAY['Fund partners', 'Local pension funds', 'High Net Worth Individuals (HNWIs)', 'Development Finance Institutions (DFIs)', 'Bilateral agencies', 'Impact investing family offices', 'Donors/philanthropy', 'Corporates'], -- target_capital_sources
  ...
  'Others',                                                     -- top_sdg_2
  'Others',                                                     -- top_sdg_3
  ARRAY['Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Consideration', 'Investment Requirement', 'Investment Requirement', 'Investment Consideration', 'Investment Consideration']::text[], -- gender_considerations_investment
  ...
  ARRAY['Preferred equity (e.g. certain rights above those available to common equity holders)', 'Convertible notes', 'Senior debt', 'Shared revenue/earnings instruments'], -- investment_forms
  ARRAY['Sector agnostic', 'Agriculture / Food supply chain', 'Energy / Renewables / Green Mobility', 'Water and Sanitation'], -- target_sectors
  ...
  ARRAY['Stabilize operations of existing portfolio companies', 'New pipeline investments through existing vehicle', 'Technical assistance to support portfolio enterprises'], -- raising_capital_2021
  ...
  '2021-02-12 17:00:46'::timestamp,                            -- completed_at
  now(),                                                        -- created_at
  now()                                                         -- updated_at
FROM auth.users u
WHERE u.email = 'e.arthur@wangaracapital.com';
